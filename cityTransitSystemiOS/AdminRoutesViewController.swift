...
class AdminRoutesViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var btnTrashIcon: UIBarButtonItem!
    ...
    @IBAction func btnShowEditing(_ sender: UIBarButtonItem) {
        
        self.btnTrashIcon.style = .plain
        
        if(self.tblAllRoutes.isEditing == true)
        {
            self.tblAllRoutes.isEditing = false
            self.btnTrashIcon.title = "Delete"
        }
        else
        {
            self.tblAllRoutes.isEditing = true
            self.btnTrashIcon.title = "Done"
        }
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
           ...
           // remove data from dict and array
            ref.child("routes").child(self.tblIdArray[indexPath.row]).removeValue()
            self.routesDict.removeValue(forKey: self.tblIdArray[indexPath.row])
            self.tblIdArray.remove(at: indexPath.row)
            self.tblAllRoutes.deleteRows(at: [indexPath], with: .fade)
            self.tblAllRoutes.reloadData()
